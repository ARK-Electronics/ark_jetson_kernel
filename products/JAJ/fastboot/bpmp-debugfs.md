...
## R39.2.1 / Linux 6.8 port

The helper selects a separate patch by the exact R39 `kernel-noble` source hash;
it also verifies `bpmp-debugfs.c` and `include/soc/tegra/bpmp.h`. R36 source hashes
and its patch remain unchanged. R39 retains the vendor VDK exclusion and its new
`suspended` transport flag / `suspend_noirq` / `resume_noirq` handling. The
freezable workqueue drains before the device's noirq suspend phase.

R39 can create NUMA-named `bpmp0` directories with a separate global `bpmp`
symlink. The asynchronous path conservatively falls back to the stock synchronous
function when the BPMP device has a NUMA node, avoiding assumptions about that
alias's ownership and teardown. The usual `NUMA_NO_NODE` layout keeps the
existing asynchronous behavior. Verify the completion message on the actual
board before attributing a timing gain to this parameter. No R36 timing is
claimed for this port, and no clocks, power domains, resets or thermal control
are deferred.

```sh
python3 scripts/patch_bpmp_debugfs.py \
  --kernel-dir staging/JAJ/Linux_for_Tegra/source/kernel/kernel-noble \
  --mode check
```
