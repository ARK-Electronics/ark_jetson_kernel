...
This directory holds the release-specific profiles for JAJ, PAB and PAB_V3 on R36.5.0 and R39.2.1.
...
## JetPack 7.2.1 / R39.2.1

Use the separately pinned R39 profile with the matching R39 BSP:

```sh
scripts/build_fast_boot_uefi.sh --bsp R39.2.1 \
  --build-dir staging/uefi-r39.2.1
```

The default remains R36.5.0 for existing callers. `r39.2.1/uefi-sources.lock`
records all commits from NVIDIA's `r39.2.1` release combination; its profile uses
`CONFIG_BUILD_MINIMAL`, which replaces R36's `CONFIG_BUILD_EMBEDDED`. It explicitly
retains NVMe/ext4, firmware TPM, OP-TEE persistent variables, Secure Boot support,
and direct firmware management/ESRT. The FMP system image type matches R39's
`t23x_general` profile. The builder uses Ubuntu 24.04 and records the exact image,
compiler, Python packages and resolved configuration actually used. R39 uses the
explicit Stuart `--target RELEASE` option; the old `UEFI_RELEASE_ONLY` environment
flag alone is no longer sufficient.

New artifacts include checksummed `artifact-release.json`. The staging helper
requires this metadata for R39, compares it with `nv_tegra_release`, and rejects
cross-release artifacts before changing the staged BSP. Legacy unlabeled R36
artifacts remain acceptable only for R36.5.0. R39 stages the firmware into
`bootloader/uefi_bins/uefi_t23x_general.bin`, selected by `p3767.conf.common` for
`jetson-orin-nano-devkit-super`; R36 uses `bootloader/uefi_jetson.bin`. The MB1 BCT
and four audited Orin NX/Nano super BPMP filenames are shared, but their contents
come from each BSP.

R39 builds also apply the reviewed `r39.2.1/uefi-single-boot-order.patch` to
the pinned `PlatformBootManagerLib/PlatformBm.c`. NVIDIA's single-application
path otherwise appends the embedded launcher after saved boot options. An A-only
firmware update preserves those options, including paths created by the general
firmware that may not resolve in this reduced image. The first R39 candidate
stopped after ReadyToBoot. The corrected firmware passed warm and cold boot on
JAJ: `BootCurrent=0008` selected the embedded `Boot Application`, and `BootOrder`
started with `0008,0001,0000`, retaining the stock NVMe and setup entries behind
it. The exact originally failing entry remains an inference because BootOrder
was not recorded before the first failed boot.

The patch inserts a new embedded launcher first, or promotes its existing entry
without deleting or reordering the other entries. Normal shell/fastboot
registration still appends. Promotion changes only `BootOrder`; `BootNext`, existing
`Boot####` entries and security variables remain intact. Single-boot load/start
failures retain NVIDIA's halt behavior but now print the failure code and status
even in RELEASE builds. Recovery access is necessary: ReadyToBoot disables the
firmware watchdog and marks the active boot chain good before the image loads,
so an image-load failure can halt without automatically selecting the other slot.

The builder checks the complete original, patched and patch-file hashes, restores
the source before publishing artifacts, and includes the patch and
`source-patches.json` provenance. Fresh R39 staging requires the checksummed
`artifact-release.json` marker `source_fix: "r39-single-boot-order-v1"`.
Rebuild older cached R39 artifacts; they are deliberately rejected for new
staging. Existing staged manifests remain verifiable and restorable for rollback.
Regression tests compile the actual patched registration and failure callbacks,
covering new and already-appended launchers, preserved entry order, variable
errors and malformed failure payloads. The corrected hardware boot also confirmed
`Timeout=0`, current/active slot A, normal fallback slot B, and successful native
bootloader validation with ESRT. These checks do not establish enrolled-key
preservation or validate PAB/PAB_V3 hardware.

The R39 build rejects `--skip-uefi-ffc-pcie`: that source patch and its enumeration
regression fixture are audited only for R36.5. The prior R36 experiment showed no
reliable timing improvement. All R39 UEFI and Linux PCIe controllers remain
available. R36 timing results do not establish R39 boot time or readiness.
