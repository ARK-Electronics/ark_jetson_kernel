...
class PatchTests(unittest.TestCase):
    ...
    def test_r39_pair_selects_its_patch_and_checks_private_api_sources(self):
        before = b'/* R39 firmware fixture */\nint option = 0;\n'
        after = b'/* R39 firmware fixture */\nint option = 1;\n'
        r39_patch = self.root / 'r39.patch'
        r39_patch.write_text(''.join(difflib.unified_diff(
            before.decode().splitlines(True), after.decode().splitlines(True),
            fromfile='a/bpmp.c', tofile='b/bpmp.c')))
        companion = self.root / 'jaj/include/soc/tegra/bpmp.h'
        companion.parent.mkdir(parents=True)
        companion.write_bytes(b'private API fixture')
        profile = {'original': hashlib.sha256(before).hexdigest(),
                   'patched': hashlib.sha256(after).hexdigest(), 'patch': r39_patch,
                   'companions': {'include/soc/tegra/bpmp.h': hashlib.sha256(companion.read_bytes()).hexdigest()}}
        self.source.write_bytes(before)
        with patch.object(helper, 'R39', profile):
            self.assertEqual(helper.update(self.root / 'jaj', 'apply'), 'patched')
            self.assertEqual(self.source.read_bytes(), after)
            companion.write_bytes(b'changed API fixture')
            with self.assertRaisesRegex(ValueError, 'companion source checksum mismatch'):
