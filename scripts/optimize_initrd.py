#!/usr/bin/env python3
"""Precompute audited NVIDIA R36.5/R39.2.1 initramfs module indexes.
...
# R39 ships an unpopulated base archive with repeated directory records. Permit
# this only for its exact vendor image during source checking, before NVIDIA's
# updater normalizes the archive. Optimizer inputs still require unique records.
AUDITED_UNNORMALIZED_SOURCES = {
    "5e52dc0b1be70a3650a2aeb0e350108fd877b5cb0074325f0f748e630d5566e8",
}
R39_STOCK_INIT_SHA256 = "7aee924ad7206877f089b7a91fc66a0190b9b930a4b5d642400117a65ebb71ad"
R39_POLLING_INIT_SHA256 = "61726f405f9d1672296f76f338f760ce4d85ba6439a190e81dbbea541177b102"
RELEASE_POLICIES = {
    "36.5.0": {"stamp": r"^# R36 \(release\), REVISION: 5\.0,", "kmod": 29,
               "kernel": r"5\.15\.185-tegra(?:[-+][A-Za-z0-9_.+-]+)?"},
    "39.2.1": {"stamp": r"^# R39 \(release\), REVISION: 2\.1,", "kmod": 31,
               "kernel": r"6\.8\.12(?:-[0-9]+)?-tegra(?:[-+][A-Za-z0-9_.+-]+)?"},
}


def release_policy(stamp):
    for release, policy in RELEASE_POLICIES.items():
        if re.search(policy["stamp"], stamp, re.M):
            return release, policy
    raise ValueError("Only audited NVIDIA L4T R36.5.0 and R39.2.1 initrds are supported")


def read_newc(data, allow_duplicate_directories=False):
    entries = []
    names = {}
    offset = 0
    while True:
        ...
        if normalized in names:
            previous = names[normalized]
            if not (allow_duplicate_directories and stat.S_ISDIR(fields[1]) and
                    stat.S_ISDIR(previous[1]) and size == previous[6] == 0 and
                    fields[1:4] == previous[1:4]):
                raise ValueError(f"Duplicate archive entry: {normalized}")
        names[normalized] = fields
...
def optimize_root_polling(init, release="36.5.0"):
    """Change only the two audited loops; retain mount and discovery budgets."""
    if release == "36.5.0":
        source_hash, output_hash = STOCK_INIT_SHA256, POLLING_INIT_SHA256
    elif release == "39.2.1":
        source_hash, output_hash = R39_STOCK_INIT_SHA256, R39_POLLING_INIT_SHA256
    else:
        raise ValueError("Unsupported root-polling release")
    if hashlib.sha256(init).hexdigest() != source_hash:
    ...
    if hashlib.sha256(updated).hexdigest() != output_hash:
        raise ValueError("Root-polling /init output checksum mismatch")
    return updated



def builtin_module_names(payload):
    """Parse the explicit build-produced builtin manifest, never missing files."""
    names = set()
    for line in payload.decode().splitlines():
        if not re.fullmatch(r"[A-Za-z0-9_./+-]+\.ko", line) or \
                line.startswith("/") or ".." in PurePosixPath(line).parts:
            raise ValueError("Invalid modules.builtin entry")
...
def optimize(args):
    ...
    release, policy = release_policy(args.l4t_release.read_text())
    for tool in ("depmod", "modprobe"):
        if not run([tool, "--version"]).startswith(f"kmod version {policy['kmod']}\n"):
            raise ValueError(f"L4T {release} requires matching kmod {policy['kmod']}; "
                             "use Ubuntu 22.04 for R36 or Ubuntu 24.04 for R39")
    ...
    polled_init = optimize_root_polling(init["payload"], release)
    ...
    if not re.fullmatch(policy["kernel"], version):
    ...
    if release == "39.2.1":
        required.add("uas")
    # A custom R39 kernel may compile these drivers in. Accept only explicit
    # entries from its matching modules.builtin, which is included in the
    # runtime integrity manifest; never infer builtins from missing .ko files.
    builtin = set()
    if release == "39.2.1":
        entry = by_name.get(f"{module_prefix}{version}/modules.builtin")
        if entry is not None:
            builtin = builtin_module_names(entry["payload"])
    if not required <= names | builtin:
    ...
    with tempfile.TemporaryDirectory(prefix="ark-initrd-depmod-") as directory:
        ...
        for module in sorted(names | (required & builtin)):
...
'''.encode()
    ...
    print(f"Optimized L4T {release} initrd for {version}: {len(modules)} unchanged modules")
