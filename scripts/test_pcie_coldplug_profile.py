...
class ColdplugTests(unittest.TestCase):
    ...
    def test_noble_preserves_combined_priority_command_and_gpu_enablements(self):
        self.vendor.write_text("\n".join(profile.UDEV_TRIGGER_DIRECTIVES_NOBLE) + "\n")
        ...
        arguments = shlex.split(commands[1].removeprefix("ExecStart="))
        self.assertEqual(arguments[:5], ["-udevadm", "trigger", "--type=all", "--action=add",
                                        "--prioritized-subsystem=module,block,tpmrm,net,tty,input"])
        self.assertEqual(arguments[5:], ["--sysname-match=" + p for p in
                                        profile.sysname_complement(profile.JAJ_PCIE_SYSNAME)])
        self.apply()
        self.restore()
        self.assertEqual(self.files(), self.original)

    def test_noble_vendor_unit_requires_matching_release_metadata(self):
        self.vendor.write_text("\n".join(profile.UDEV_TRIGGER_DIRECTIVES_NOBLE) + "\n")
        self.assert_rejected_without_changes("L4T R39.2.1")
        (self.root / "etc/nv_tegra_release").write_text("# R39 (release), REVISION: 2.1,\n")
        (self.root / "etc/os-release").write_text('ID=ubuntu\nVERSION_ID="22.04"\n')
        self.assert_rejected_without_changes("Ubuntu 24.04")
