# Historical R36.5.0 fast-boot validation record

**The hardware timings, completed board builds and deployment artifact hashes
in the historical sections belong to R36.5.0 / JetPack 6. None is an R39.2.1 /
JetPack 7.2.1 result.**
The current [build procedure](fast_boot.md) describes the R39 port separately;
its firmware, kernel, modules, Noble rootfs and application require their own
...
## R39.2.1 port checks, without a timing claim

The initrd and rootfs helpers now use separate R39.2.1/Ubuntu 24.04 guards while
retaining the R36 checks. The R39 `/init` source hash is
`7aee924ad7206877f089b7a91fc66a0190b9b930a4b5d642400117a65ebb71ad`;
the reviewed polling-only result is
`61726f405f9d1672296f76f338f760ce4d85ba6439a190e81dbbea541177b102`.
Final optimized archive hashes depend on the matched kernel/module payload and
must be recorded from the actual deployment build.

Host checks exercised kmod 31 against an offline fixture containing the stock
R39 Image and a 12-module dependency subset, preserving the module payloads and
the full vendor init script outside the audited changes. That subset was a
helper test, not a deployable product initrd. Required built-in drivers need
explicit entries in the matching `modules.builtin` manifest.

Read-only audits of the vendor-installed Noble rootfs accepted the distinct
systemd/udev 255 USB and coldplug configuration, and the rsyslog/AppArmor logging
configuration. Tests cover exact rollback, mismatched releases, altered vendor
inputs and preservation of NVIDIA GPU services. Repeat those audits after final
ARK-OS provisioning and package changes. These checks establish helper behavior;
they do not establish boot time, USB reconnect, persistent logs, GPU inference,
camera capture or application readiness on R39 hardware.

## Scope and completed builds

The following completed builds cover JAJ, PAB and PAB_V3 on **L4T R36.5.0 with NVMe**.
...
SHA-256 for the historical R36.5.0 builds and firmware candidates. These identify
